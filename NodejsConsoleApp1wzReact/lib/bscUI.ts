...
function renderTemplate4vue(template: string, data: any): string {
    let txt = "";
    ...
    while (curIdx < lines.length) {
        ...
        if (line.includes("*vfor=")) {
            ...
            const list:any[] = getFieldV2(data, key, []);
...
function renderTemplate4Agl(template: string, data: Data): string {
    let txt = "";
    const lines = template.split("\n");
    let curIdx = 0;
...
function extractListName4vue(input: string): string {
 ...
 * @returns ?????????????????? null
 */
function extractListName4agl(input: string): string | null {
    // ?????????? *ngFor=list ??? list ??
 ...
 * @returns ?????????
 */
function getFieldV2(data: any, key: string, defaultValue: any): any {
    // ??????????????? null?undefined???????
...
// ?????
function renderRowTmplt(rowTmplt: string, list: any[]): string {
...
function rendMustacheFrmTmpltTxt(data: any, line: string): string {
